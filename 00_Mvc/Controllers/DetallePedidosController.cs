...
namespace _00_Mvc.Controllers
{
    public class DetallePedidosController : Controller
    {
        ...
        public ActionResult Index(int? id, string madre, string nombreMadre)
        {
            ...
            IList<DetallePedidoDto> detallePedidoDto = null;
            ...
            detallePedidoDto = service.List(id, madre);
            ViewBag.Message = "";
            if (madre != null && madre != "")
            {
                ViewBag.Message = "DetallePedidos del " + madre + ": " + nombreMadre;
            }
            return View(detallePedidoDto);
        }
        ...
        public ActionResult Details(int? id)
        {
            ...
            detallePedidoDto = service.Detail(id.Value);
            //Fin Nuevo
            //Esto como estaba:
        ...
        public ActionResult Create(DetallePedidoDto detallePedidoDto)
        {
            DetallePedidosService service = null;
            if (ModelState.IsValid)
            {
                service = new DetallePedidosService();
                bool ok = false;
                ok = service.Create(detallePedidoDto.);
                if (ok == true)
                {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(DetallePedidoDto detallePedidoDto)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                DetallePedidoDto buscada = service.Detail(detallePedidoDto.OrderID);
                ...
                ok = service.Edit(detallePedidoDto);
                if (ok == true)
                {
                    //Si esto sucede, entonces llama al método "Index"
                    return RedirectToAction("Index");
        ...
        {            //Nuevo
            ...
            //Lo utilizamos para llegar a su método Detail 
            //Y, así rellenar nuestro DetallePedido detallePedido
            detallePedidoDto = service.Detail(id);
            //Fin Nuevo
            bool ok = false;
            ok = service.Delete(detallePedidoDto);
